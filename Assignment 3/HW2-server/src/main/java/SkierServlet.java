...
public class SkierServlet extends HttpServlet {
  ...
  private final static String QUEUE_NAME_SKIER = "SkierServletPostQueue";
  private final static String QUEUE_NAME_RESORT = "ResortServletPostQueue";


  private Gson gson  = new Gson();

  private ObjectPool<Channel> pool1;
//  private ObjectPool<Channel> pool2;

  public void init() {
    this.pool1 = new GenericObjectPool<Channel>(new ConnectionPoolFactory());
//    this.pool2 = new GenericObjectPool<Channel>(new ConnectionPoolFactory2());
  }
  ...
  protected void doPost(HttpServletRequest req,
    ...
    if (!isUrlValid(urlParts) || urlParts[2].equals("vertical")) {
      ...
      try {
        ...
        liftInfo.addProperty("vertical", skierReqBody.getLiftID()*10);

        Channel channel = null;
//        Channel channel2 = null;
        try {
          channel = pool1.borrowObject();
          ...
          channel.basicPublish("", QUEUE_NAME_SKIER, null, liftInfo.toString().getBytes());
          channel.basicPublish("", QUEUE_NAME_RESORT, null, liftInfo.toString().getBytes());
//          System.out.println(" [x] Sent '" + liftInfo.toString() + "'");
          ...
          try {
            if (null != channel) {
              pool1.returnObject(channel);
//              pool2.returnObject(channel2);
            }
          } catch (Exception e) {
